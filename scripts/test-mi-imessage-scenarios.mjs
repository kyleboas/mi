...
assert.equal(decide('run detect').action, 'start', 'pipeline run starts from a direct request');
assert.equal(decide('run ingest').action, 'start', 'ingest starts from a direct request');
assert.equal(decide('run report').action, 'start', 'report starts from a direct request');
assert.equal(decide('rescore pending candidates').action, 'start', 'rescore starts from a direct request');
assert.equal(decide('approve candidate #123').action, 'start', 'candidate approval starts from a direct request');
assert.equal(decide('reject candidate #123').action, 'start', 'candidate rejection starts from a direct request');
assert.equal(decide('edit report #123').action, 'start', 'report edits start from a direct request');
assert.equal(decide('restart cron').action, 'start', 'cron restart starts from a direct request');
assert.equal(decide('delete candidate #123').action, 'ask', 'destructive delete still asks for confirmation');
...
assert.equal(directiveThenUrl.action, 'start', 'URL after pending directive starts work');
...
const confirmAfterUrl = decide('yes', [
  ...
  assistant('On it. I’ll follow up here.', 'imessage-work-ack', 4),
]);
assert.equal(confirmAfterUrl.action, 'chat', 'confirmation after URL has no pending work once worker acked');
...
assert.equal(askFirst.action, 'start', 'ask-first mode still starts non-risky direct work');
