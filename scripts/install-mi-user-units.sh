...
# No daemon drop-ins were generated by the reviewed legacy unit. An absent
# directory is the reviewed empty bundle; an existing directory must be a
# safe, genuinely empty directory. This rejects unknown names, duplicate
# directives, controls, symlinks, execution directives, and all other bytes
# rather than attempting to parse untrusted systemd input.
assert_known_legacy_daemon_dropins() {
  local target="$UNIT_DIR/mi-daemon.service.d" entry
  ...
  for entry in "${entries[@]}"; do
    fail "refusing unknown legacy daemon drop-in: $entry"
  done
}
legacy_daemon=0
assert_replaceable_unit() {
  ...
  [[ "$target" != "$UNIT_DIR/mi-daemon.service" ]] || assert_known_legacy_daemon_dropins
}
