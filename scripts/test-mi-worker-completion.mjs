...
const timing = sanitizeTurnEvent({ stage: 'coordinator-first-assistant-delta', outcome: 'ok', route: 'v2', modelProfile: 'coordinator', turn: 'turn', durationMs: 250, prompt: 'must not survive', path: '/private/path' });
assert.deepEqual(Object.keys(timing).sort(), ['durationMs', 'modelProfile', 'outcome', 'route', 'schema', 'stage', 'ts', 'turn'].sort(), 'timing instrumentation retains only allowlisted metadata');
assert.equal(timing.stage, 'coordinator-first-assistant-delta');
assert.equal(timing.modelProfile, 'coordinator');
assert.equal(sanitizeTurnEvent({ stage: 'untrusted-stage', modelProfile: 'untrusted-profile' }).stage, 'terminal', 'unallowlisted timing values fail closed');
