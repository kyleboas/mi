...
assert.equal(v2RiskClassification('Reply exactly: iMessage check passed.').kind, 'safe', 'same-conversation exact replies do not require confirmation');
assert.equal(v2RiskClassification('reply to Alice: iMessage check passed.').kind, 'confirm', 'external-recipient replies require confirmation');
...
assert.equal(v2RouteDecision({ message: 'Reply exactly: iMessage check passed.', workspace: { root: '/tmp/work', cwd: '/tmp/work' } }).kind, 'coordinator', 'same-conversation exact replies use the guarded coordinator');
