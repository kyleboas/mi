...
assert.match(cli, /if \(isTaskNeedsInput\(task\)\)[\s\S]*task\.needsUserReason === 'stopped by Escape'[\s\S]*already in needs input[\s\S]*dismissTaskFromList\(task\)[\s\S]*else if \(isTaskActive\(task\)\)/, 'Esc on Esc-paused task keeps it in needs input instead of dismissing it');
...
assert.match(daemon, /const preserveStoredWorking = storedWorking && scannedComplete/, 'daemon keeps tasks in working despite passive completed session scans');
assert.match(daemon, /status: \(preserveStoredTerminal \|\| preserveStoredWorking\) \? task\.status : \(activeSession\.status \|\| task\.status\)/, 'daemon preserves stored task state when matching stale passive session scans');
