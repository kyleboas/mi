...
assert.match(cli, /function readSessionFinalOutput\(sessionFile: string, options: \{ full\?: boolean \} = \{\}\)[\s\S]*readSessionOutputSync\(sessionFile, options\.full === true\)[\s\S]*!isNonFinalAssistantText\(text\)[\s\S]*return text/, 'agent view skips queued-goal inactive sentinel when showing session final output');
...
assert.match(cli, /function taskSectionMovedMs\(task: MiTask\)[\s\S]*section === 'needs input'[\s\S]*task\.updatedAt \|\| task\.lastEventAt[\s\S]*section === 'working'[\s\S]*task\.continuedAt \|\| task\.updatedAt[\s\S]*task\.finishedAt \|\| task\.updatedAt/, 'section ordering uses the time a task moved into that section');
assert.match(cli, /\.sort\(\(a, b\) => taskSectionMovedMs\(b\.task\) - taskSectionMovedMs\(a\.task\) \|\| taskStartedMs\(b\.task\) - taskStartedMs\(a\.task\)\)/, 'sections sort by latest section move time so reply/Esc moves are immediate');
...
assert.match(cli, /function taskActivitySymbol\(task: MiTask, animated = true, frameIndex = 0\)[\s\S]*isTaskNeedsInput\(task\)\) return '●'[\s\S]*'○'[\s\S]*PI_SPINNER_FRAMES\[frameIndex % PI_SPINNER_FRAMES\.length\]/, 'main Mi TUI marks needs-input with filled circle and working tasks with animation');
...
assert.match(cli, /if \(task && isTaskActive\(task\)\)[\s\S]*task\.status = 'paused'[\s\S]*stopTaskInList\(task\)[\s\S]*else if \(task\)[\s\S]*dismissedCompactAgentKeys\.add\(key\)[\s\S]*Removed \$\{taskName\(task\)\} from list/, 'Esc stops active compact agents into needs input and removes inactive selected agents');
...
assert.match(daemon, /MI_DAEMON_LOCK_START_GRACE_MS[\s\S]*waiting for daemon lock[\s\S]*continue/, 'daemon waits for a just-starting lock owner instead of killing it and racing duplicate daemons');
...
assert.match(daemon, /const preserveStoredWorking = storedWorking && scannedComplete && storedWorkingIsNewerThanScan\(task, activeSession\)/, 'daemon only keeps working state over older completed session scans');
...
assert.match(daemon, /Task needs \$\{miUserName\(\)\}: \$\{merged\.sessionName \|\| merged\.name \|\| merged\.id\}/, 'daemon pushes needs-User task transitions to main thread');
...
assert.match(daemon, /notifiedNeedsUserAt/, 'daemon de-duplicates needs-User main-thread notifications');
