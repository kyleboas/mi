...
assert.match(cli, /function taskFullOutputFallback\(task: MiTask\)[\s\S]*task\.sessionFile[\s\S]*taskDisplayText\(task\) \|\| task\.progress \|\| 'No result yet\.'[\s\S]*taskFinalOutput\(task, \{ full: true \}\)[\s\S]*const fullLastOutput = fullLastOutputMode && task \? taskFullOutputFallback\(task\) : ''/, 'agent view expands available task output in full output mode without reparsing full session files on each keypress');
...
assert.match(cli, /fullLastOutput && task[\s\S]*renderFullOutputLines\(task, fullLastOutput \|\| 'No result yet\.', width\)[\s\S]*terminal\/tmux scrollback[\s\S]*lines\.push\(\.\.\.outputLines\)[\s\S]*shouldFlushScrollback[\s\S]*lines\.slice\(-height\)/, 'full last output mode renders complete output once for native scrollback and then redraws only the visible tail');
