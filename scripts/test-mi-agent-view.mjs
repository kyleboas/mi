...
assert.match(cli, /function renderFullOutputLines\(task: MiTask, fallbackText: string, width: number\)[\s\S]*const transcript = sessionFile \? readSessionFullTranscript\(sessionFile\) : \[\][\s\S]*item\.role === 'user' \? renderPiUserMessage\(item\.text, width\) : renderPiLastOutputMessage\(item\.text, width\)[\s\S]*const outputLines = renderFullOutputLines\(task, fullLastOutput \|\| 'No result yet\.', width\)[\s\S]*terminal\/tmux scrollback[\s\S]*lines\.push\(\.\.\.outputLines\)/, 'full last output mode renders full transcript for native scrollback with pi message formatting');
