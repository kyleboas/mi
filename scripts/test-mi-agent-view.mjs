...
assert.match(cli, /function taskStatus\(task: MiTask\)[\s\S]*task\.needsKyle && task\.needsKyleReason === 'stopped by Escape'[\s\S]*return 'paused'/, 'stopped needs-Kyle tasks display as paused');
...
assert.match(cli, /else if \(isTaskActive\(task\)\)[\s\S]*task\.status = 'paused'[\s\S]*task\.needsKyle = true[\s\S]*task\.needsKyleReason = 'stopped by Escape'[\s\S]*pendingTaskUpdates\.set\(taskKey, \{ status: 'paused'[\s\S]*stopTaskInList\(task\)/, 'Esc stops any active selected task and pins paused state during refresh');
...
assert.match(daemon, /function detectNeedsKyle\(task\)/, 'daemon detects tasks needing user attention');
...
assert.match(daemon, /Task needs \$\{miUserName\(\)\}: \$\{merged\.sessionName \|\| merged\.name \|\| merged\.id\}/, 'daemon pushes needs-Kyle task transitions to main thread');
