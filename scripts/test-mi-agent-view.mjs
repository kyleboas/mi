...
assert.match(cli, /session: \$\{task\.sessionName \|\| ''\} \$\{task\.sessionFile \|\| ''\}[\s\S]*const prUrls = extractPrUrlsFromTask\(task\);[\s\S]*`PR: \$\{prUrls\.join\(' '\)\}`[\s\S]*isTaskNeedsInput\(task\)[\s\S]*renderPiAssistantMessage\(taskNeedsInputQuestion\(task\), width\)[\s\S]*const lastInput = taskLastInput\(task\);[\s\S]*renderPiUserMessage\(lastInput, width\)/, 'task detail shows session and PR links before needs-input prompt and last input');
...
assert.match(cli, /data !== '\\r' && data !== '\\n'[\s\S]*\\x1b\\\[13;5/, 'raw Enter is not treated as CSI-u Ctrl-M, but CSI-u Ctrl-M is accepted');
assert.match(cli, /inputMode = 'reply'[\s\S]*status = `Reply to \$\{taskName\(task\)\}`/, 'raw Enter keeps reply semantics instead of toggling multi-select');
...
assert.match(cli, /if \(isTaskNeedsInput\(task\)\)[\s\S]*pendingTaskUpdates\.delete\(key\)[\s\S]*dismissTaskFromList\(task\)[\s\S]*else if \(isTaskActive\(task\)\)/, 'Esc on paused/needs-input task clears it instead of stopping again');
...
assert.match(daemon, /status: "inactive",[\s\S]*needsUser: false,[\s\S]*interactive pi session stopped/, 'daemon marks naturally stopped busy interactive pi sessions inactive, not needs input');
...
assert.match(daemon, /status: preserveStoredTerminal \? task\.status : \(activeSession\.status \|\| task\.status\)/, 'daemon preserves terminal stored task state when matching a stale busy pi session');
...
assert.match(daemon, /task\.needsUser && task\.needsUserReason === "stopped by Escape"[\s\S]*status === "error"[\s\S]*return \{ needsUser: false/, 'daemon only marks Esc-paused or failed tasks as needs input');
