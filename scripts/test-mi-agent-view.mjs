...
assert.match(cli, /const MI_TASK_POLL_MS = Number\(process\.env\.MI_TASK_POLL_MS \|\| 1000\)/, 'agent view uses a near-real-time default polling cadence');
...
assert.match(cli, /const transcript = task\.sessionFile \? readSessionFullTranscript\(task\.sessionFile\) : \[\][\s\S]*item\.role === 'user' \? renderPiUserMessage\(item\.text, width\) : renderPiLastOutputMessage\(item\.text, width\)[\s\S]*Rendering the entire[\s\S]*transcript on every keypress makes typing laggy[\s\S]*outputLines\.slice\(start, start \+ outputViewportHeight/, 'full last output mode renders a bounded transcript viewport with pi message formatting');
...
assert.match(cli, /fullLastOutput && task[\s\S]*renderPiLastOutputMessage\(fullLastOutput \|\| 'No result yet\.', width\)[\s\S]*const viewportHeight = Math\.max\(1, contentHeight - lines\.length\)[\s\S]*outputLines\.slice\(start, start \+ outputViewportHeight/, 'full last output mode renders a bounded viewport so typing stays fast');
...
assert.match(cli, /fullLastOutputMode && !inputBuffer && \(isPageUpKey\(data\) \|\| isPageDownKey\(data\)\)[\s\S]*fullLastOutputScroll = Math\.max\(0, fullLastOutputScroll \+ \(isPageUpKey\(data\) \? 12 : -12\)\)/, 'full output mode uses PageUp/PageDown to page inside bounded output');
