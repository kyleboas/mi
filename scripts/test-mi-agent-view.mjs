...
assert.match(daemon, /if \(openPiSession\) \{[\s\S]*const openFields = \{ openPiSession: true, openPiPid: openPiSession\.pid, openPiInput: openPiSession\.openPiInput, bridgeSocket: piBridgeSocketPath\(task\.actualSessionFile \|\| task\.sessionFile\) \}[\s\S]*includes\(status\)\) return \{ \.\.\.task, \.\.\.openFields, status: "running", finishedAt: undefined \}[\s\S]*return \{ \.\.\.task, \.\.\.openFields \}/, 'daemon keeps open interactive pi sessions linked so mi agents replies queue into them even when idle');
