...
assert.match(cli, /function renderFullOutputLines\(task: MiTask, fallbackText: string, width: number\)[\s\S]*const transcript = sessionFile && !isTaskActive\(task\) && !isStoppedPiSessionWithoutFinal\(task\) \? readSessionFullTranscript\(sessionFile\) : \[\][\s\S]*item\.role === 'user' \? renderPiUserMessage\(item\.text, width\) : renderPiLastOutputMessage\(item\.text, width\)[\s\S]*const outputLines = renderFullOutputLines\(task, fullLastOutput \|\| 'No result yet\.', width\)/, 'full last output mode renders full transcript for completed tasks and avoids stale transcript output for active tasks');
...
assert.match(cli, /fullLastOutput && task[\s\S]*renderFullOutputLines\(task, fullLastOutput \|\| 'No result yet\.', width\)[\s\S]*const viewportHeight = Math\.max\(1, height - lines\.length - footerLines\.length\)[\s\S]*const visibleOutput = outputLines\.slice\(Math\.max\(0, end - viewportHeight\), end\)[\s\S]*lines\.push\(\.\.\.visibleOutput\)/, 'full output mode renders a terminal-sized viewport so typing does not redraw the whole stored output');
...
assert.match(cli, /fullLastOutputMode && !inputBuffer && \(isPageUpKey\(data\) \|\| isPageDownKey\(data\)\)[\s\S]*fullLastOutputScroll = Math\.max\(0, Math\.min\(fullLastOutputMaxScroll, fullLastOutputScroll \+ \(isPageUpKey\(data\) \? page : -page\)\)\)/, 'full output mode pages its own viewport while up/down still switch the selected task');
