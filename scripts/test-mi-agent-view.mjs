...
assert.match(cli, /if \(isTaskActive\(task\)\)[\s\S]*task\.status = 'paused'[\s\S]*task\.needsKyle = true[\s\S]*task\.needsKyleReason = 'stopped by Escape'[\s\S]*stopTaskInList\(task\)/, 'Esc stops any active selected task and moves it to needs input instead of dismissing it');
