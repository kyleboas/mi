...
This manual command refuses tracked, staged, and untracked changes before it contacts GitHub. It accepts only the `origin` SSH or HTTPS URL for `kyleboas/mi`, fetches only `origin/main`, reports the prior ref and commit, and preserves that commit in a uniquely named local `mi-deploy-rollback-<UTC timestamp>` branch. On its first run it creates the dedicated `deploy/mi` branch from `origin/main`; later runs only fast-forward that branch. It never switches to or modifies local `main`. If `deploy/mi` is not an ancestor of `origin/main`, it fails closed and the operator must resolve that branch manually. The exact validation sequence is `npm ci`, `npm run build`, `npm test`, the focused tick canary, then the compiled CLI tick canary. It never resets, cleans, rewrites user branches, or activates timers/outbound jobs.
