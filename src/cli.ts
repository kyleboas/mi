...
function isStoppedPiSessionWithoutFinal(task: MiTask) {
  const isPiSession = task.source === 'pi-session' || String(task.id || '').startsWith('pi-session:') || Boolean(task.sessionFile);
  if (!isPiSession || !task.needsUser) return false;
  const reason = String(task.needsUserReason || '');
  if (/Pi session is no longer running and no final assistant response was recorded/i.test(reason)) return true;
  if (task.status && String(task.status).toLowerCase() !== 'paused') return false;
...
function stoppedPiSessionNotice(task: MiTask) {
  const reason = String(task.needsUserReason || '').trim();
  if (/Pi session is no longer running and no final assistant response was recorded/i.test(reason)) return reason;
...
function taskFinalOutput(task: MiTask, options: { full?: boolean } = {}) {
  if (isStoppedPiSessionWithoutFinal(task)) return stoppedPiSessionNotice(task);
...
function textFromSessionMessage(message: any) {
  const content = message?.content;
  if (typeof content === 'string') return sanitizeVisibleAssistantText(content);
...
function readSessionOutputSync(sessionFile: string, full = false) {
  return full ? readFileSync(sessionFile, 'utf8') : readSessionTailSync(sessionFile);
}

function readSessionFinalOutput(sessionFile: string, options: { full?: boolean } = {}) {
  try {
    ...
    for (const line of records) {
      ...
      if (text && !isNonFinalAssistantText(text)) outputs.push(text);
    }
    if (options.full) return outputs.join('\n\n');
    return outputs.at(-1) || '';
  } catch {}
  return '';
}
...
function readSessionFullTranscript(sessionFile: string) {
  try {
    ...
    for (const line of raw.trim().split(/\r?\n/)) {
      ...
      const content = record.message?.content;
      if (role === 'assistant' && Array.isArray(content) && content.some((part: any) => part?.type === 'toolCall')) continue;
...
function normalizedTaskName(task: MiTask) {
  return String(task.sessionName || task.name || '').trim().toLowerCase().replace(/\s+/g, ' ');
}

function isGenericTaskName(name: string) {
  return !name || name === 'user' || name === 'mi session' || name === 'pi session' || name === 'recent mi session';
}

function taskCwdKey(task: MiTask) {
  return String(task.cwd || '').replace(/\/+$/, '');
}

function normalizedTaskLastInput(task: MiTask) {
  return normalizeLastInputText(task.lastInput || '').toLowerCase().slice(0, 500);
}

function taskSessionIdentityKeys(task: MiTask) {
  return [...new Set([task.sessionId, sessionFingerprint(task), task.sessionFile, task.actualSessionFile].filter(Boolean).map(String))];
}

function tasksSameIdentity(a: MiTask, b: MiTask) {
  const aKeys = new Set(taskIdentityKeys(a));
  if (taskIdentityKeys(b).some((key) => aKeys.has(key))) return true;
  const sameCwd = taskCwdKey(a) === taskCwdKey(b);
  if (!sameCwd) return false;
  const aName = normalizedTaskName(a);
  const bName = normalizedTaskName(b);
  if (aName && aName === bName && !isGenericTaskName(aName)) return true;
  // No session-key or explicit-name match: two tasks that each point at their
  // own pi session are distinct tasks even when cwd/lastInput look alike
  // (e.g. two detect runs from the same directory). The lastInput fallback
  // only links a task that has no session identity yet to its session.
...
async function listResumeSessions() {
  if (process.env.MI_AGENT_RENDER_TEST_RESUME_ERROR) throw new Error(process.env.MI_AGENT_RENDER_TEST_RESUME_ERROR);
  if (process.env.MI_AGENT_RENDER_TEST_RESUME_SESSIONS) {
    const sessions = JSON.parse(readFileSync(process.env.MI_AGENT_RENDER_TEST_RESUME_SESSIONS, 'utf8')) as MiTask[];
    return sessions.sort((a: MiTask, b: MiTask) => taskUpdatedMs(b) - taskUpdatedMs(a) || taskStartedMs(b) - taskStartedMs(a));
  }
...
async function miAgentsCommand() {
  ...
  let selected = 0;
  let closed = false;
  const defaultAgentStatus = '^F full output • ^M multi-select • Esc clear task';
  ...
  function rememberTaskDisplayOrder(newTasks: MiTask[], previousTasks: MiTask[] = tasks) {
    ...
    for (const task of newTasks) {
      ...
      if (!key || taskDisplayOrder.has(key)) continue;
      ...
      if (previousOrder !== undefined) taskDisplayOrder.set(key, previousOrder);
      else unseen.push(task);
    }
    if (unseen.length > 0) {
    ...
    for (const [key] of [...taskDisplayOrder.entries()]) {
      ...
      if (!newTasks.some((task) => stableTaskKey(task) === key || (previous && tasksSameIdentity(task, previous)))) taskDisplayOrder.delete(key);
    }
  }

  function taskDisplayOrderValue(task: MiTask) {
    const key = stableTaskKey(task);
  ...
  async function refresh() {
    ...
    try {
      ...
      status = inputMode === 'normal' && !agentSubmitting && !resumeMode ? (multiSelectMode ? multiSelectStatus() : defaultAgentStatus) : status;
    } catch (error) {
      status = error instanceof Error ? error.message : String(error);
    }
  ...
  function toggleMultiSelectMode() {
    ...
    if (!multiSelectMode) selectedTaskKeys.clear();
  ...
  function clearSelectedTasksFromList() {
    ...
    if (!renderTestMode) for (const task of toDismiss) void dismissTaskFromList(task).catch((error) => { status = error instanceof Error ? error.message : String(error); requestRender(true); });
  ...
  function logRenderTestDispatch(entry: Record<string, unknown>) {
    const path = process.env.MI_AGENT_RENDER_TEST_DISPATCH_LOG;
    if (!renderTestMode || !path) return false;
    appendFileSync(path, `${JSON.stringify(entry)}
`);
    return true;
  }

  async function sendNativeAgentSlashCommand(value: string) {
    const task = replyTarget || selectedTask();
    if (!task) { status = `Select a task before using ${slashCommandName(value)}`; requestRender(); return; }
    const taskId = task.id || task.sessionFile || task.sessionName || task.name;
    if (!taskId) { status = `Selected task has no id for ${slashCommandName(value)}`; requestRender(); return; }
    ...
    void sendTaskSocketRequest({ type: 'continue_worker', taskId, message: value, model: agentModelWithThinking(), background: !waitForDone, preserveStatus: true }, waitForDone ? 120000 : 30000)
      .then(async () => {
        ...
        await refresh();
        if (waitForDone) status = 'Increment summarized and marker advanced';
        else if (isMarker) status = 'Marker set';
        else status = `Sent ${command} to selected task`;
  ...
  async function openResumeMenu() {
    ...
    if (!resumeMode) {
      // The user cancelled (Esc/^C) while sessions were loading.
      resumeLoading = false;
      resumeEnterPending = false;
      resumeSessions = [];
      requestRender();
    ...
    if (resumeSession) {
      ...
      if (key) selectedResumeKeys.add(key);
    }
    resumeLoading = false;
  ...
  function renderAgentLines(width = cols()): string[] {
    ...
    const lines: string[] = [];
    const task = selectedTask();
    // Avoid re-reading the whole session file on every render frame; the real
    // transcript render below is cached, so the fallback only needs cheap
    ...
    if (lines.length > contentHeight) lines.splice(contentHeight);
  ...
  function collectHeadlessPiText(stdout: string, stderr: string) {
    let text = '';
    for (const line of stdout.split('\n')) {
      ...
      try {
        ...
        else if (event.type === 'message_end' && event.message?.errorMessage && !text) text += event.message.errorMessage;
        else if (event.type === 'error' && event.error && !text) text += String(event.error);
      } catch {
        if (!text) text += `${line}\n`;
      }
    }
  ...
  async function runHeadlessPiCommand(args: string[], cwd: string): Promise<{ text: string }> {
    return await new Promise((resolve, reject) => {
      ...
      child.on('close', (code) => {
        ...
        else reject(new Error(text || `pi exited ${code}`));
      });
    });
  }

  async function runHeadlessAgentSlashCommand(value: string) {
    const task = replyTarget || selectedTask();
  ...
  async function submitAgentInput() {
    ...
    if (inputMode === 'reply') {
      ...
      agentSubmitting = true;
      requestRender();
      let turn: { body: string; model: string };
      try {
        turn = await applyAgentPiCycle(value);
      } catch (error) {
        if (taskKey) {
          pendingTaskUpdates.delete(taskKey);
          pendingTaskUpdateStartedAt.delete(taskKey);
      ...
      if (!turn.body) {
        // Cycle-only input (e.g. "zz") switches the model without sending a
        // follow-up; undo the optimistic running state set above.
        if (taskKey) {
          pendingTaskUpdates.delete(taskKey);
          pendingTaskUpdateStartedAt.delete(taskKey);
        }
  ...
  function isAgentTextPasteInput(data: string) {
    return data.includes('\x1b[200~') || (data.length > 1 && data.includes('\n') && /[^\r\n]/.test(data) && !data.includes('\x1b'));
  }

  function handleAgentTextPasteInput(data: string) {
    ...
    if (data === '\x1b') {
      ...
      if (task) {
        if (isTaskNeedsInput(task)) {
          ...
          tasks = tasks.filter((entry) => (key ? stableTaskKey(entry) !== key : entry !== task));
          optimisticTasks = optimisticTasks.filter((entry) => (key ? stableTaskKey(entry) !== key : entry !== task));
          clampTaskSelection();
          ...
          if (!renderTestMode) void dismissTaskFromList(task).catch((error) => { status = error instanceof Error ? error.message : String(error); requestRender(true); });
          ...
          clampTaskSelection();
          if (!renderTestMode) void stopTaskInList(task).then(() => refresh()).catch((error) => { status = error instanceof Error ? error.message : String(error); requestRender(true); });
        } else {
          const key = stableTaskKey(task);
  ...
  async function runAgentRenderTest() {
    ...
    if (fixturePath) tasks = dedupeTasksByStableKey(JSON.parse(readFileSync(fixturePath, 'utf8')) as MiTask[]);
    rememberTaskDisplayOrder(tasks, []);
    clampTaskSelection();
    selected = navigationTaskIndexes()[0] ?? selected;
    const width = cols();
    const frames: Array<{ event: string; selected: number; selectedTask?: string; inputMode: string; status: string; lines: string[] }> = [];
    const snapshot = (event: string) => frames.push({
      event,
      selected,
    ...
    const keyForEvent = (event: string) => {
      if (event === 'up') return '\x1b[A';
      if (event === 'down') return '\x1b[B';
      if (event === 'pageUp') return '\x1b[5~';
      if (event === 'pageDown') return '\x1b[6~';
      if (event === 'enter') return '\n';
      if (event === 'space') return ' ';
      if (event === 'escape') return '\x1b';
      if (event === 'ctrlL') return '\x0c';
      if (event === 'ctrlF') return '\x06';
      if (event === 'ctrlC') return '\x03';
      if (event === 'ctrlM' || event === 'multiSelect') return '\r';
      if (event === 'shiftTab') return '\x1b[Z';
      if (event.startsWith('text:')) return event.slice('text:'.length);
      if (event.startsWith('paste:')) return event.slice('paste:'.length).replace(/\\n/g, '\n');
      return event;
    };
    snapshot('initial');
    for (const event of (process.env.MI_AGENT_RENDER_TEST_EVENTS || 'down,down,pageDown,pageUp,up,enter,escape').split(',').map((item) => item.trim()).filter(Boolean)) {
      if (event.startsWith('add:')) {
        ...
        const task: MiTask = { id: `render-test-${name}`, name, cwd: HOME, status: 'running', progress: `test-added ${name}`, startedAt: ts, updatedAt: ts };
        ...
        const selectedKey = previouslySelectedTask ? stableTaskKey(previouslySelectedTask) : '';
        const previousTasks = tasks;
        tasks = dedupeTasksByStableKey(JSON.parse(readFileSync(event.slice('reload:'.length), 'utf8')) as MiTask[]);
        rememberTaskDisplayOrder(tasks, previousTasks);
        const nextSelected = findTaskIndexByIdentity(tasks, previouslySelectedTask, selectedKey);
        if (nextSelected >= 0) selected = nextSelected;
        clampTaskSelection();
...
function miUserPossessive({ capitalize = false } = {}) {
  ...
  if (name.toLowerCase() === 'the owner') return capitalize ? "The owner's" : "the owner's";
...
type MiTask = {
  ...
  notifiedNeedsUserAt?: string;
  notifiedPausedAt?: string;
  prUrls?: string[];
  sessionFile?: string;
...
function stripLeadingVisibleWhitespace(line: string) {
  let index = 0;
  let output = '';
...
let piResourceCommandLoad: Promise<SlashCommand[]> | undefined;
...
async function loadPiResourceSlashCommands(cwd = process.cwd()): Promise<SlashCommand[]> {
...
function getCachedPiResourceSlashCommands(cwd = process.cwd()): SlashCommand[] {
  const now = Date.now();
  ...
  if (!piResourceCommandLoad) {
    piResourceCommandLoad = new Promise<SlashCommand[]>((resolve) => {
...
type MiAgentsCommandClass = typeof MI_COMMAND_CLASSIFICATIONS[keyof typeof MI_COMMAND_CLASSIFICATIONS]['class'];
  ...
  return {
    async getSuggestions(lines: string[], cursorLine: number, cursorCol: number, options: { signal: AbortSignal; force?: boolean }) {
      const resourceCommands = getCachedPiResourceSlashCommands(process.cwd())
        .filter((command) => Boolean((MI_COMMAND_CLASSIFICATIONS as Record<string, unknown>)[`/${command.name}`]));
...
function isCtrlFShortcut(data: string) {
...
function isCtrlMShortcut(data: string) {
...
async function miTuiCommand(initial = '') {
  ...
  type MiSurface = 'main-tui' | 'imessage' | 'web' | 'agents';
  type MiSurfaceMode = 'converse' | 'delegate';
  type MiWorkerRoutingDecision = { start: boolean; reason: string };

  function miSurfaceMode(surface: MiSurface): MiSurfaceMode {
    if (surface === 'main-tui' || surface === 'imessage') return 'converse';
    return 'delegate';
  }

  function normalizedMiMessageText(text: string) {
    return String(text || '').trim().toLowerCase();
  }

  function miMessageHasLocalWorkTarget(text: string) {
  ...
  function miMessageLooksActionable(text: string) {
    const normalized = normalizedMiMessageText(text);
  ...
  function miMessageLooksLikeComplaint(text: string) {
    const normalized = normalizedMiMessageText(text);
    if (!normalized || normalized.startsWith('/')) return false;
  ...
  function miMessageLooksConversational(text: string) {
    const normalized = normalizedMiMessageText(text);
    if (!normalized || normalized.startsWith('/')) return false;
    if (/^(?:stop|cancel|never mind|nevermind|thanks|thank you|ok|okay|yes|no|got it|cool|nice)[.!?\s]*$/.test(normalized)) return true;
    if (/\b(?:what\s+time\s+is\s+it|current\s+time|what\s+day\s+is\s+it|what\s+date\s+is\s+it)\b/.test(normalized)) return true;
    if (/\b(?:why|did)\b[\s\S]{0,120}\b(?:handoff|hand\s*off|pass(?:ed)?(?:\s+that)?\s+(?:on|along)|worker)\b/.test(normalized)) return true;
    return false;
  }

  function miMessageLooksLikeInlineChat(text: string) {
    const normalized = normalizedMiMessageText(text);
    if (!normalized || normalized.startsWith('/')) return false;
    if (/\b(?:draft|write|rewrite|compose|wordsmith)\b/.test(normalized) && !/\b(?:code|script|app|ui|repo|file|test|tests|implementation|bug|fix)\b/.test(normalized)) return true;
    if (miMessageHasLocalWorkTarget(normalized) && (miMessageLooksActionable(normalized) || miMessageLooksLikeComplaint(normalized))) return false;
  ...
  function miEstimatedWorkScore(text: string) {
    const normalized = normalizedMiMessageText(text);
    if (!normalized || normalized.startsWith('/')) return 0;
    let score = 3;
    if (miMessageLooksActionable(normalized)) score += 10;
    if (miMessageLooksLikeComplaint(normalized)) score += 10;
    if (miMessageHasLocalWorkTarget(normalized)) score += 6;
    if (/\b(?:code|repo|project)\/[a-z0-9_.-]+|~\/code\/[a-z0-9_.-]+|\/home\/\w+\/code\/[a-z0-9_.-]+/.test(normalized)) score += 12;
    if (normalized.length > 220) score += 5;
    return score;
  }

  function miWorkerRoutingDecision(text: string): MiWorkerRoutingDecision {
    const normalized = normalizedMiMessageText(text);
    if (!normalized || normalized.startsWith('/')) return { start: false, reason: 'empty-or-command' };
    if (miMessageLooksConversational(normalized)) return { start: false, reason: 'conversation' };
    if (miMessageLooksLikeInlineChat(normalized)) return { start: false, reason: 'inline-chat' };
    ...
    if (/\b(?:worker|background\s*(?:worker|task)|handoff|hand\s*off)\b/.test(normalized) && (actionable || complaint || localTarget)) return { start: true, reason: 'explicit worker/local work' };
    if (localTarget && (actionable || complaint)) return { start: true, reason: 'repo/app work' };
    if (localTarget && score >= threshold + 6) return { start: true, reason: 'likely multi-step local work' };
    if (/\b(?:research|investigate|inspect|check|verify|debug|fix|implement|build|test)\b/.test(normalized) && score >= threshold + 6) return { start: true, reason: 'substantive task' };
  ...
  async function buildBackgroundWorkerPromptFromMi(text: string, decision = miWorkerRoutingDecision(text)) {
  ...
  function compactMiAckText(value: string, max = 120) {
    const clean = String(value || '').replace(/\s+/g, ' ').trim();
    if (clean.length <= max) return clean;
    return `${clean.slice(0, max - 1).trim()}…`;
  }

  function miHandoffActionSummary(text: string) {
    const normalized = normalizedMiMessageText(text);
    if (/routing|handoff|hand\s*off|worker/.test(normalized)) return 'I’ll tighten Mi routing/hand-off behavior';
    if (/robotic|awkward|repetitive/.test(normalized)) return 'I’ll make that response less stiff';
    if (/typing/.test(normalized)) return 'I’ll track down the typing-state bug';
    if (/notification|pwa|pushover/.test(normalized)) return 'I’ll adjust the notification setup';
    if (/logo|favicon/.test(normalized)) return 'I’ll swap the logo';
    if (/plus|icon|button|centered|aligned|alignment/.test(normalized)) return 'I’ll fix the button alignment';
    if (/detect|candidate|tacticsjournal|research/.test(normalized)) return 'I’ll check the project and pull the actual list';
    const quoted = compactMiAckText(text);
    return quoted ? `I’ll work on “${quoted}”` : 'I’ll take care of it';
  }

  function miHandoffReasonSentence(decision: MiWorkerRoutingDecision) {
  ...
  async function startBackgroundWorkerFromMi(text: string, decision = miWorkerRoutingDecision(text)) {
  ...
  function renderMiLines(width = cols()): string[] {
    ...
    while (lines.length < height) lines.unshift('');
    return lines;
  }
  ...
  async function cycleThinking() {
    ...
    const currentLevel = THINKING_LEVELS.includes(miState?.thinkingLevel as ThinkingLevel) ? miState!.thinkingLevel as ThinkingLevel : (miThinkingLevel || 'low');
...
async function loadPiCommandInventory(cwd = process.cwd()) {
  ...
  for (const rawCommand of [...builtin, ...resources]) {
    const command = rawCommand as any;
    ...
    byName.set(command.name, {
      ...
      classification: (MI_COMMAND_CLASSIFICATIONS as Record<string, { class: MiAgentsCommandClass }>)[slashName]?.class || null,
      blockedReason: (MI_COMMAND_CLASSIFICATIONS as Record<string, { class: MiAgentsCommandClass; description: string }>)[slashName]?.class === 'blocked' ? (MI_COMMAND_CLASSIFICATIONS as Record<string, { description: string }>)[slashName]?.description : undefined,
    });
  }
  for (const [slashName, info] of Object.entries(MI_COMMAND_CLASSIFICATIONS)) {
    const name = slashName.replace(/^\//, '');
    if (!byName.has(name)) byName.set(name, { name, slash: slashName, source: 'mi-classification', description: info.description, classification: info.class, blockedReason: info.class === 'blocked' ? info.description : undefined });
  }
...
async function cronCommand(args: string[]) {
  ...
  if (subcommand === 'check' || subcommand === 'tick') {
    ...
    if (ran.length === 0) console.log('No due Mi crons.');
    else for (const item of ran) console.log(`${item.name}: ${item.status}`);
    return;
  }
  if (subcommand === 'remove' || subcommand === 'delete' || subcommand === 'rm') {
    const name = args[1];
    if (!name) throw new Error('cron name required');
