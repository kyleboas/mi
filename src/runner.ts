...
import { assistantPath, parseAssistantMarkdown, validateAssistantFile, type AssistantRunRequest, type AssistantRunResult } from './assistant.js';
import { addApproval, addWorkerResult, createRunRecord, finishRunRecord, writeRunRecord, type Trigger } from './primitives.js';
import { assertRuntimeCannotSelfModify } from './safety.js';
import { runWorker, type WorkerRequest, type WorkerResult } from './workers.js';
...
export type RunnerWorkerRequest = WorkerRequest;
export type RunnerWorkerResult = WorkerResult;

export async function startWorker(request: RunnerWorkerRequest): Promise<RunnerWorkerResult> {
...
export async function startWorkerForRun(runId: string, request: RunnerWorkerRequest): Promise<RunnerWorkerResult> {
...
export function checkRuntimeInstructionEdit(targetPath: string) {
  return assertRuntimeCannotSelfModify(targetPath);
}

export async function checkAssistant(name: string): Promise<RunnerCheck> {
  ...
  try {
    ...
    exists = true;
    if (!markdown.trim()) issues.push('assistant file is empty');
    try {
...
export async function runAssistant(request: AssistantRunRequest): Promise<AssistantRunResult> {
  const check = await checkAssistant(request.name);
  if (!check.ok) {
    const trigger: Trigger = request.trigger === 'manual' ? { manual: true } : { event: request.trigger };
