...
export function safeNotificationText(text: string) {
...
export async function notifyImessage(title: string, message: string, options: { requireEnabled?: boolean } = {}) {
  if (options.requireEnabled !== false && !/^(1|true|yes|on)$/i.test(process.env.MI_PROACTIVE_IMESSAGE_NOTIFY || process.env.MI_IMESSAGE_NOTIFY || '')) return { skipped: true };
...
export async function notifyPushover(title: string, message: string) {
  if (!pushoverEnabled()) return { skipped: true };
...
export async function notify(title: string, message: string) {
  ...
  if (!(imessage as any).skipped) results.imessage = imessage;
  ...
  if (!(pushover as any).skipped) results.pushover = pushover;
