...
export type ProactiveResult = { ok: boolean; result?: any; error?: string };

function flueCmd() {
  if (process.env.FLUE_CMD) return process.env.FLUE_CMD;
...
export async function runFlueProactive(agent: string, payload: unknown): Promise<ProactiveResult> {
  if (process.env.FLUE_ENABLED === 'false') return { ok: false, error: 'FLUE_ENABLED=false' };
  ...
  return await new Promise((resolve) => {
    ...
    const finish = (result: ProactiveResult) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
    ...
    child.on('close', (code) => {
      ...
      if (code !== 0 || !raw) return finish({ ok: false, error: stderr.trim() || `Flue exited ${code}` });
      try {
