...
export type UploadToken = { token: string; createdAt: string; expiresAt: string; used: boolean };
...
async function readTokens(): Promise<UploadToken[]> {
  try { return JSON.parse(await readFile(TOKEN_PATH, 'utf8')) as UploadToken[]; } catch { return []; }
}

async function writeTokens(tokens: UploadToken[]) {
...
export async function createUploadLink(baseUrl = uploadPublicBaseUrl()) {
  ...
  const now = Date.now();
  const record: UploadToken = { token, createdAt: new Date(now).toISOString(), expiresAt: new Date(now + UPLOAD_TTL_MS).toISOString(), used: false };
...
export function safeImageName(original: string, contentType: string) {
  ...
  if (!ext) throw new Error('Only JPEG, PNG, GIF, and WebP images are allowed.');
...
export function sniffImage(buffer: Buffer, contentType: string) {
...
export async function consumeUpload(token: string, body: Buffer, contentType: string, originalName = 'image') {
  if (!/^[A-Za-z0-9_-]{32}$/.test(token)) throw new Error('Bad upload token.');
  if (body.length === 0) throw new Error('Empty upload.');
  if (body.length > UPLOAD_MAX_BYTES) throw new Error(`Image is too large; max ${UPLOAD_MAX_BYTES} bytes.`);
  const type = contentType.split(';')[0].trim().toLowerCase();
  if (!sniffImage(body, type)) throw new Error('Upload is not a valid supported image.');
  const now = Date.now();
  const tokens = await readTokens();
  const record = tokens.find((item) => item.token === token);
  if (!record || record.used || Date.parse(record.expiresAt) <= now) throw new Error('Upload link expired or already used.');
  const filename = safeImageName(originalName, type);
  await mkdir(UPLOAD_DIR, { recursive: true });
  const dest = resolve(UPLOAD_DIR, filename);
  if (!dest.startsWith(resolve(UPLOAD_DIR) + '/')) throw new Error('Unsafe upload path.');
...
export async function cleanupUploads(now = Date.now()) {
  ...
  try {
    for (const name of await readdir(UPLOAD_DIR)) {
      ...
      if (now - s.mtimeMs > UPLOAD_RETENTION_MS) await rm(path, { force: true });
    }
  } catch {}
}
