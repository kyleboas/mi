...
export function clearSession(res: Response, sid?: string) {
...
export function requireAuth(req: Request, res: Response, next: NextFunction) {
  ...
  if (!configured) return res.status(503).json({ error: 'auth not configured: set ASSISTANT_TOKEN or ASSISTANT_PASSWORD' });

  if (req.header('authorization') === `Bearer ${configured}`) return next();
  ...
  if (process.env.CSRF_DISABLED !== 'true' && req.method !== 'GET' && req.method !== 'HEAD') {
    if (req.header('x-csrf-token') !== session.csrf) return res.status(403).json({ error: 'bad csrf' });
  }

  next();
}

export function currentCsrf(req: Request) {
  const sid = req.cookies?.assistant_session;
  const session = sid ? sessions.get(sid) : undefined;
