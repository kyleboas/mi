import { assistantPath, type AssistantDraft } from './assistant.js';
...
export type BuilderEditRequest = {
  name: string;
  change: string;
  currentMarkdown: string;
};

function inferName(description: string) {
  const lower = description.toLowerCase();
  if (lower.includes('production')) return 'production';
  if (lower.includes('repo')) return 'repo-maintenance';
  if (lower.includes('calendar')) return 'calendar';
  if (lower.includes('inbox') || lower.includes('email')) return 'inbox';
  return 'assistant';
}

export function draftAssistant(request: BuilderRequest): AssistantDraft {
...
export function proposeAssistantEdit(request: BuilderEditRequest): AssistantDraft {
  return {
    name: request.name,
    path: assistantPath(request.name),
    markdown: `${request.currentMarkdown.trim()}\n\n<!-- Proposed builder change for human review: ${request.change.trim()} -->\n`,
  };
}
