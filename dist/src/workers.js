...
function collectPiText(stdout) {
    ...
    for (const line of stdout.split('\n')) {
        ...
        try {
            ...
            if (event.type === 'message_end' && event.message?.errorMessage && !text)
...
async function runPi(prompt, cwd, tools, timeoutMs = 180_000) {
...
export async function runPiInspectWorker(request) {
...
export async function runPiRepairWorker(request) {
...
export async function runWorker(request) {
    if (request.kind === 'pi.inspect')
        return runPiInspectWorker(request);
    if (request.kind === 'pi.repair')
        return runPiRepairWorker(request);
