...
function taskNewestMs(task) {
    return Math.max(Date.parse(task.updatedAt || '') || 0, Date.parse(task.lastEventAt || '') || 0, Date.parse(task.continuedAt || '') || 0, Date.parse(task.startedAt || '') || 0, Date.parse(task.finishedAt || '') || 0);
}
...
function tasksSameIdentity(a, b) {
    ...
    if (taskSessionIdentityKeys(a).length > 0 && taskSessionIdentityKeys(b).length > 0)
        return false;
...
function findTaskIndexByIdentity(list, selectedTask, selectedKey = selectedTask ? stableTaskKey(selectedTask) : '') {
    if (!selectedTask && !selectedKey)
        return -1;
...
async function miAgentsCommand() {
    ...
    function agentEditorText(expanded = false) {
        const maybeExpanded = agentEditor;
    ...
    async function startAgentSlashBackgroundTask(value) {
    ...
    async function submitAgentInput() {
        ...
        if (exitFullOutputForReply)
            exitFullOutputAfterReply();
    ...
    function onData(data) {
        ...
        if (isCtrlFShortcut(data)) {
            ...
            if (fullLastOutputMode && selected < 0 && tasks.length > 0)
                selected = navigationTaskIndexes()[0] ?? 0;
            fullLastOutputScroll = 0;
        ...
        if (resumeMode && !inputBuffer) {
            ...
            for (const key of keys) {
                ...
                if (key === '\x1b' || key === '\x03') {
                    ...
                    if (resumeMultiSelectMode && resumeSessions[resumeSelected])
    ...
    pollTimer = setInterval(() => {
        ...
        if (!hasLiveWork && Date.now() - lastIdlePollAt < MI_IDLE_TASK_POLL_MS)
            return;
        if (!hasLiveWork)
            lastIdlePollAt = Date.now();
        void refresh();
    }, MI_TASK_POLL_MS);
    clockTimer = setInterval(() => {
        if (!resumeLoading && !tasks.some(isTaskActive) && !optimisticTasks.some(isTaskActive))
            return;
        requestRender();
    }, MI_AGENT_CLOCK_MS);
...
function isTerminalKeyRelease(data) {
...
async function startMiDaemonWithSystemd() {
    ...
    if (!unit || process.env.MI_DAEMON_SYSTEMD === '0' || !existsSync('/usr/bin/systemctl'))
        return false;
    if (!await runQuiet('/usr/bin/systemctl', ['--user', 'cat', unit], 3000))
        return false;
...
async function miTuiCommand(initial = '') {
    ...
    function queuedMessageLines(width) {
    ...
    function handleEscapeKey() {
        if (restoreQueuedMessagesToEditor())
            return;
        if (pending) {
    ...
    if (process.env.MI_TUI_RENDER_TEST === '1') {
        ...
        if (process.env.MI_TUI_RENDER_TEST_EVENT === 'escape')
            handleEscapeKey();
