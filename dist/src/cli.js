...
function localTimeToUtcIso(hour, minute, timeZone = 'America/New_York') {
    const nowDate = new Date();
    const nowParts = new Intl.DateTimeFormat('en-US', { timeZone, year: 'numeric', month: '2-digit', day: '2-digit' }).formatToParts(nowDate).reduce((acc, part) => {
        if (part.type !== 'literal')
            acc[part.type] = part.value;
        return acc;
    }, {});
    ...
    if (guess.getTime() <= nowDate.getTime())
        guess = new Date(guess.getTime() + 24 * 60 * 60_000);
    return guess.toISOString();
}
async function maybeHandleLocalIntent(message) {
    ...
    if (meridiem === 'am' && hour === 12)
        hour = 0;
    if (hour < 0 || hour > 23 || minute < 0 || minute > 59 || !text)
        return undefined;
...
async function taskCommand(args) {
    ...
    if (name === 'reply') {
        ...
        const result = await sendSocketRequest(payload, 30_000).catch(async (error) => {
            if (existsSync(MI_SOCKET_PATH) && !isSocketConnectionRefused(error))
                throw error;
            await startMiDaemon();
...
async function cronCommand(args) {
    ...
    if (sub === 'remove') {
        const name = args[1];
        if (!name)
            throw new Error('usage: mi cron remove <name>');
    ...
    if (sub === 'add') {
        ...
        if (!name || !every || !command)
            throw new Error('usage: mi cron add <name> --every 1h [--cwd <path>] -- <command>');
...
function userBgLine(text, width) {
...
function isSocketConnectionRefused(error) {
...
async function startMiDaemon() {
    ...
    if (existsSync(MI_SOCKET_PATH))
        await rm(MI_SOCKET_PATH, { force: true });
...
async function miTuiCommand(threadId = 'main', initial = '') {
    ...
    function refreshActiveTasks() {
        sendSocketRequest({ type: 'list_tasks' }, 5000)
            .then((result) => {
            activeTasks = (result.tasks || []).filter(isActiveTask);
            requestRender();
        })
            .catch(() => undefined);
    }
    ...
    function inputWrappedPlainLines(width) {
        const wrapWidth = Math.max(1, width);
        const text = inputDisplayText();
        const lines = [];
        for (let i = 0; i < text.length; i += wrapWidth)
            lines.push(text.slice(i, i + wrapWidth));
    ...
    function slashSuggestionLines(width) {
        ...
        if (start > 0 || start + visible.length < suggestions.length)
            lines.push(fgDim(truncateText(`  (${slashSelectedIndex + 1}/${suggestions.length})`, width)));
        return lines;
    }
    ...
    function inputDisplayLines(width) {
        const wrapWidth = Math.max(1, width);
        ...
        if (hasCursorRoom)
            rendered.push(truncateText(`${last}${INVERSE}${CURSOR_CELL}${RESET_INVERSE}`, width));
        else
            rendered.push(truncateText(last, width), `${INVERSE}${CURSOR_CELL}${RESET_INVERSE}`);
        return rendered;
    }
    function inputCursorPosition(width, inputRows, inputStartRow) {
        const lines = inputWrappedPlainLines(width);
    ...
    function activeTaskLines(width) {
        return activeTasks
            .slice(0, 5)
    ...
    async function askOne(text) {
        ...
        try {
            ...
            if (threadId === 'main') {
                ...
                for (const message of await readThreadMessages(threadId, 300))
    ...
    async function runLocalSlashCommand(text) {
        if (text === '/quit') {
            cleanup();
            return true;
        }
    ...
    function onData(buffer) {
        ...
        if (data === '\x03') {
            if (pending || messageQueue.length > 0) {
                ...
                const now = Date.now();
                if (now - lastSigintTime < 500 && !inputLine.trim())
                    cleanup();
                else {
                    inputLine = '';
                    lastSigintTime = now;
...
async function main() {
    ...
    if (command === 'ui')
        return miTuiCommand('main', args.join(' '));
    if (command === 'chat' || command === 'open')
        return miTuiCommand(args[0] || 'main');
    if (command === 'read')
        return showThread(args[0] || 'main');
    if (command === 'upload')
