...
import { parseAssistantMarkdown } from './assistant.js';
...
export function scheduledTriggers(triggers) {
    return triggers.filter((trigger) => 'every' in trigger);
}
export async function runScheduledAssistant(name) {
    ...
    if (schedules.length === 0)
        return { skipped: true, reason: 'assistant has no scheduled triggers' };
...
export async function explainSchedule(name) {
    const markdown = await readFile(`assistants/${name}.md`, 'utf8');
    const assistant = parseAssistantMarkdown(markdown, `assistants/${name}.md`);
