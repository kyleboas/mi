...
async function writeThreadIndex(threads) {
    await mkdir(threadsDir, { recursive: true });
...
export async function createTempThread(title) {
    await ensureThreads();
    const threads = await readThreadIndex();
    let base = `temp-${safeThreadId(title)}`;
    let candidate = base;
    let suffix = 2;
    while (threads.some((thread) => thread.id === candidate))
        candidate = `${base}-${suffix++}`;
    const ts = now();
    const record = { id: candidate, title, kind: 'temporary', createdAt: ts, updatedAt: ts, unread: 0 };
    threads.push(record);
    await writeThreadIndex(threads);
    await appendThreadMessage(candidate, 'system', `Temporary conversation created: ${title}`, { unread: false, source: 'mi' });
    return record;
}
export async function appendThreadMessage(threadId, role, text, options = {}) {
    await ensureThreads();
    const threads = await readThreadIndex();
    ...
    if (message.unread && role === 'assistant')
        record.unread += 1;
    await writeThreadIndex(threads);
    return message;
}
export async function readThreadMessages(threadId = 'main', limit) {
    await ensureThreads();
    try {
...
export async function compactThread(threadId = 'main', keep = Number(process.env.MI_COMPACT_KEEP || 100)) {
    await ensureThreads();
    const messages = await readThreadMessages(threadId);
...
export async function archiveThread(threadId) {
    if (threadId === 'main')
        throw new Error('main thread cannot be archived');
    await ensureThreads();
    const threads = await readThreadIndex();
    const record = threads.find((thread) => thread.id === threadId);
    if (!record)
