...
function summarizeEvent(event) {
    ...
    if (event.type === 'tool_execution_end')
        return `tool done: ${event.toolName}${event.isError ? ' error' : ''}`;
    if (event.type === 'auto_retry_start')
        return `retrying: ${event.errorMessage || ''}`;
...
export async function runPiReadOnlyStream(prompt, emit) {
...
export async function runPiReadOnly(prompt) {
    let latest = { text: '', trace: [] };
