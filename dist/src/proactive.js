...
function approvalSummary(approval) {
    ...
    if (/Matched risky action pattern/i.test(reason) && prompt)
        return prompt;
...
function formatFailedCronLine(cron) {
...
async function recentWorkTasks() {
    const miTasks = await readJsonArray(join(miStateDir, 'tasks.json'));
    const webWorkers = (await readJsonArray(join(stateDir, 'web-workers.json')))
        .map((worker) => ({
        ...worker,
...
function isBriefableTask(task) {
    const title = briefTaskTitle(task).toLowerCase();
    const detail = String(task.text || task.progress || task.lastInput || '').toLowerCase();
    if (/^(continue|ok|okay|thanks|yes|no)$/.test(title.trim()))
        return false;
    if (/^running shell command$/.test(detail.trim()))
        return false;
...
function projectFromTask(task) {
    ...
    if (/38-0|38and0/.test(text))
        return '38-0 / 38and0.com';
    if (/tacticsjournal|detect|research/.test(text))
        return 'Tactics Journal research/detect pipeline';
    if (/mi|briefing|worker|handoff|web chat|routing|assistant/.test(text))
        return 'Mi assistant / background workers';
    if (/railrat|train-112/.test(text))
        return 'Railrat train 112 monitor';
    if (/cloudflare|budget/.test(text))
        return 'Cloudflare / budget guard';
...
async function projectLines() {
    ...
    for (const task of tasks.slice(0, 120)) {
        ...
        if (project)
            projects.set(project, Math.max(projects.get(project) || 0, taskTime(task)));
    }
...
function sendSocketRequest(payload, timeoutMs = 30000) {
...
async function readAutoActionState() {
    try {
...
async function writeAutoActionState(state) {
...
async function consumeReadOnlyTriageBudget() {
    ...
    if (maxPerDay <= 0)
        return { allowed: false, reason: 'daily budget disabled' };
    ...
    if ((state.readOnlyTriageToday || 0) >= maxPerDay)
...
async function startRepairWorker(notice) {
    if (!notice.repairPrompt)
        return null;
...
async function readMonitorHealthState() {
    const parsed = await readJsonObject(monitorHealthPath);
    return { version: 1, monitors: parsed.monitors && typeof parsed.monitors === 'object' ? parsed.monitors : {} };
}
async function writeMonitorHealthState(state) {
...
function recoveryLine(observation) {
    return `- ${observation.title} is healthy again.`;
}
function makeMonitorRepairPrompt(observation) {
    if (observation.repairPrompt)
        return observation.repairPrompt;
    return [
...
function repairAllowed(observation, previous, nowMs) {
    if (!observation.repairable)
        return false;
    if (observation.status !== 'stale' && observation.status !== 'degraded')
        return false;
    if (!repairableMonitorReasons.has(observation.reason))
        return false;
    if (previous?.nextRepairAfter && Date.parse(previous.nextRepairAfter) > nowMs)
        return false;
    return true;
}
...
function defaultTacticsRegistryEntries() {
...
function parseMonitorRegistry(markdown) {
    const entries = [];
    for (const line of markdown.split('\n')) {
        const trimmed = line.trim();
        if (!trimmed.startsWith('|') || trimmed.includes('---') || /^\|\s*id\s*\|/i.test(trimmed))
            continue;
        const cells = trimmed.split('|').slice(1, -1).map((cell) => cell.trim());
        if (cells.length < 8)
            continue;
        const [id, title, type, source, freshness, , allowedAutoActions] = cells;
        if (!id || !title || !type || !source)
            continue;
        if (type !== 'health_sidecar' && type !== 'mi_crons')
            continue;
        entries.push({
            id,
            title,
...
async function readMonitorRegistry() {
...
async function observeTacticsHealthStep(entryOrStep) {
    ...
    try {
        ...
        if (human)
            return { id: entry.id, title, status: 'human-required', reason, detail: checkedAt ? `checked ${checkedAt}` : path, repairable: false };
        ...
        if (status === 'ok')
            return { id: entry.id, title, status: 'ok', reason: 'ok', detail: checkedAt ? `checked ${checkedAt}` : undefined };
...
async function observeMiCronHealth() {
    const crons = await readCrons().catch(() => []);
    const enabled = crons.filter((cron) => cron.enabled);
    if (enabled.length === 0)
        return [{ id: 'mi-crons:configured', title: 'Mi reminder crons', status: 'ok', reason: 'none-configured', detail: 'No Mi reminder crons are configured.' }];
    ...
    if (failed.length > 0)
        return [{ id: 'mi-crons:reminders', title: 'Mi reminder crons', status: 'degraded', reason: 'failed', detail: failed.map((cron) => cron.name).join(', '), repairable: true }];
...
async function observeConfiguredMonitors() {
    ...
    const sidecars = (await Promise.all(registry.filter((entry) => entry.type === 'health_sidecar').map(observeTacticsHealthStep))).filter((item) => Boolean(item));
    const miCrons = registry.some((entry) => entry.type === 'mi_crons') ? await observeMiCronHealth() : [];
    return [...sidecars, ...miCrons];
}
export async function configuredMonitorHealth() {
    ...
    for (const id of Object.keys(state.monitors)) {
        if (id.startsWith('tactics:') && !activeIds.has(id))
    ...
    for (const observation of observations) {
        ...
        if (transition !== 'seed' && transition !== 'unchanged') {
            ...
            else if (observation.status === 'human-required')
                humanRequired.push(observation);
            else if (transition === 'broken')
                becameBroken.push(observation);
            else
                changedBroken.push(observation);
        }
    ...
    if (becameBroken.length === 0 && changedBroken.length === 0 && recovered.length === 0 && humanRequired.length === 0)
        return null;
    const lines = [
