...
const MI_MEMORY_PATH = join(MI_ROOT, "state", "memory", "MEMORY.md");
...
function boundedMiMemory() {
	try {
		const value = readFileSync(MI_MEMORY_PATH, "utf8")
			.split("\n")
			.filter((line) => !/\b(?:token|secret|password|api[_ -]?key|authorization|bearer)\b/i.test(line))
			.join("\n")
			.trim();
		return value.slice(0, Number(process.env.MI_MEMORY_MAX_CHARS || 5000));
	} catch { return ""; }
}
