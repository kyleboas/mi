...
const PI_SESSION_SCAN_CACHE_MS = Number(process.env.MI_PI_SESSION_SCAN_CACHE_MS || 3000);
const PI_IDLE_SESSION_SCAN_CACHE_MS = Number(process.env.MI_IDLE_PI_SESSION_SCAN_CACHE_MS || 60000);
const PI_SESSION_SCAN_LIMIT = Number(process.env.MI_PI_SESSION_SCAN_LIMIT || 15);
...
async function waitForPiBridge(socketPath, timeoutMs = 5000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    if (existsSync(socketPath)) {
      try {
        await sendBridgeRequest(socketPath, { type: "health", source: "mi-agents", sourcePid: process.pid }, 500);
        return true;
      } catch {}
    }
...
async function runIncrementalWorkflowCommandInHeadlessBridge(task, message, model) {
  const sessionFile = task.actualSessionFile || task.sessionFile;
  if (!sessionFile || !isSlashCommand(message)) return false;
  const worker = createRpcProcess({ cwd: task.cwd || HOME, sessionFile, model: String(model || MI_MODEL).trim(), env: { MI_WORKER: "1" } });
  try {
...
async function continueWorker(request) {
  ...
  if (request.preserveStatus) {
    ...
    if (task.openPiSession && await runIncrementalWorkflowCommandInPiBridge(task, workerInputMessage(message, request.useGoal)).catch(() => false)) {
